...
const PasswordInfo = ({ passwordFade, activeTab, setOldPassword, setNewPassword, setPassword, matchPassword, handleInputChange}) => {
    ...
    return (
        <div className={`passwordFormContainer ${passwordFade ? "Fade" : ""} ${activeTab === "password" ? "visible" : "hidden"}`}>
            <h2>Trocar senha</h2>
            <form className="passwordForm">
                <div className="formItem">
                    <label htmlFor="oldPassword">Senha antiga</label>
                    <input type={showOldPassword ? "text" : "password"} id="oldPassword" name="oldPassword" onChange={(e) => {setOldPassword(e.target.value)}}/>
                    <box-icon type='solid' name={showOldPassword ? "lock-open" : "lock"} onClick={toggleOldPassword} id="lockIcon" size="22px"></box-icon>  
                </div>
                <div className="formItem">
